...
#define CABECALHO_MOVIMENTACAO                                                 \
  int id = blockIdx.x * blockDim.x + threadIdx.x;                              \
  if (id < quantAgentes)
#define CABECALHO_CONTATO                                                      \
  int id = blockIdx.x * blockDim.x + threadIdx.x;                              \
  if (id < indexPosicoes[indexQuadras[quantQuadras * 2 - 1]] / 4)
#define CABECALHO_TRANSICAO                                                    \
  int id = blockIdx.x * blockDim.x + threadIdx.x;                              \
  if (id < quantAgentes)

#define CABECALHO_SAIDA_ESPACIAL                                               \
  int id = blockIdx.x * blockDim.x + threadIdx.x;                              \
  if (id < indexPosicoes[indexQuadras[quantQuadras * 2 - 1]] / 4)
#define CABECALHO_SAIDAS_QUANTIDADES                                           \
  int id = blockIdx.x * blockDim.x + threadIdx.x;                              \
  if (id < quantAgentes)

#define ATOMIC_ADD_TOTAL(ciclo, d, e)                                          \
  atomicAdd(                                                                   \
      &saidaQuantidadeTotal[VEC(ciclo, d + e, COLUNAS_SAIDAS_QUANTIDADES)], 1)

#define ATOMIC_ADD_QUADRAS(ciclo, d, e)                                        \
  atomicAdd(                                                                   \
      &saidaQuantidadeQuadras[indexSaidaQuantidadeQuadras[GET_Q(id)] +         \
                              VEC(ciclo, d + e, COLUNAS_SAIDAS_QUANTIDADES)],  \
      1)
...
#define CABECALHO_MOVIMENTACAO                                                 \
  _Pragma("omp parallel for") for (int id = 0; id < quantAgentes; ++id)
#define CABECALHO_CONTATO                                                      \
  _Pragma(                                                                     \
      "omp parallel for") for (int id = 0;                                     \
                               id <                                            \
                               indexPosicoes[indexQuadras[quantQuadras * 2 -   \
                                                          1]] /                \
                                   4;                                          \
                               id++)
#define CABECALHO_TRANSICAO                                                    \
...
void inserirAgentes(int quantAgentes, TIPO_AGENTE *agentes,
                    const double *parametros, const int *quantLotes,
                    ...
                    const int *indexTrajetosFaixaEtaria, int quantidade,
                    int estado, int sexo, int idade, int *i, int k) {
  ...
  for (int j = 0; j < quantidade; ++j) {
    t = ENTRE_FAIXA(indexTrajetosFaixaEtaria[3 - idade],
                    indexTrajetosFaixaEtaria[(3 - idade) + 1], dis(gen));
...
TIPO_AGENTE *criarAgentes(int quantAgentes, const double *parametros,
  ...
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_SUSCETIVEIS_CRIANCA_MASCULINO_LOCAL,
                 SUSCETIVEL, MASCULINO, CRIANCA, &i, LOCAL);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
  ...
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 ...
                 INFECTADO, MASCULINO, CRIANCA, &i, TRAJETO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_RECUPERADOS_CRIANCA_MASCULINO_TRAJETO,
                 RECUPERADO, MASCULINO, CRIANCA, &i, TRAJETO);

  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_SUSCETIVEIS_JOVEM_MASCULINO_LOCAL,
                 SUSCETIVEL, MASCULINO, JOVEM, &i, LOCAL);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
  ...
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 ...
                 RECUPERADO, MASCULINO, JOVEM, &i, LOCAL);

  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_SUSCETIVEIS_JOVEM_MASCULINO_ALEATORIO,
                 SUSCETIVEL, MASCULINO, JOVEM, &i, ALEATORIO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
  ...
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 ...
                 INFECTADO, MASCULINO, JOVEM, &i, ALEATORIO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_RECUPERADOS_JOVEM_MASCULINO_ALEATORIO,
                 RECUPERADO, MASCULINO, JOVEM, &i, ALEATORIO);

  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_SUSCETIVEIS_JOVEM_MASCULINO_LIVRE,
                 SUSCETIVEL, MASCULINO, JOVEM, &i, LIVRE);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
  ...
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 ...
                 RECUPERADO, MASCULINO, JOVEM, &i, LIVRE);

  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_SUSCETIVEIS_JOVEM_MASCULINO_TRAJETO,
                 SUSCETIVEL, MASCULINO, JOVEM, &i, TRAJETO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
  ...
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 ...
                 INFECTADO, MASCULINO, ADULTO, &i, TRAJETO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_RECUPERADOS_ADULTO_MASCULINO_TRAJETO,
                 RECUPERADO, MASCULINO, ADULTO, &i, TRAJETO);

  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_SUSCETIVEIS_IDOSO_MASCULINO_LOCAL,
                 SUSCETIVEL, MASCULINO, IDOSO, &i, LOCAL);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
  ...
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 ...
                 RECUPERADO, MASCULINO, IDOSO, &i, LOCAL);

  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_SUSCETIVEIS_IDOSO_MASCULINO_ALEATORIO,
                 SUSCETIVEL, MASCULINO, IDOSO, &i, ALEATORIO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
  ...
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 ...
                 INFECTADO, MASCULINO, IDOSO, &i, ALEATORIO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_RECUPERADOS_IDOSO_MASCULINO_ALEATORIO,
                 RECUPERADO, MASCULINO, IDOSO, &i, ALEATORIO);

  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_SUSCETIVEIS_IDOSO_MASCULINO_LIVRE,
                 SUSCETIVEL, MASCULINO, IDOSO, &i, LIVRE);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
  ...
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 ...
                 RECUPERADO, MASCULINO, IDOSO, &i, LIVRE);

  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_SUSCETIVEIS_IDOSO_MASCULINO_TRAJETO,
                 SUSCETIVEL, MASCULINO, IDOSO, &i, TRAJETO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
  ...
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 ...
                 INFECTADO, FEMININO, CRIANCA, &i, TRAJETO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_RECUPERADOS_CRIANCA_FEMININO_TRAJETO,
                 RECUPERADO, FEMININO, CRIANCA, &i, TRAJETO);

  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_SUSCETIVEIS_JOVEM_FEMININO_LOCAL,
                 SUSCETIVEL, FEMININO, JOVEM, &i, LOCAL);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_EXPOSTOS_JOVEM_FEMININO_LOCAL, EXPOSTO,
  ...
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 ...
                 RECUPERADO, FEMININO, JOVEM, &i, LOCAL);

  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_SUSCETIVEIS_JOVEM_FEMININO_ALEATORIO,
                 SUSCETIVEL, FEMININO, JOVEM, &i, ALEATORIO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_EXPOSTOS_JOVEM_FEMININO_ALEATORIO, EXPOSTO,
                 FEMININO, JOVEM, &i, ALEATORIO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_INFECTADOS_JOVEM_FEMININO_ALEATORIO,
                 INFECTADO, FEMININO, JOVEM, &i, ALEATORIO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_RECUPERADOS_JOVEM_FEMININO_ALEATORIO,
                 RECUPERADO, FEMININO, JOVEM, &i, ALEATORIO);

  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_SUSCETIVEIS_JOVEM_FEMININO_LIVRE,
                 SUSCETIVEL, FEMININO, JOVEM, &i, LIVRE);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_EXPOSTOS_JOVEM_FEMININO_LIVRE, EXPOSTO,
  ...
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 ...
                 RECUPERADO, FEMININO, JOVEM, &i, LIVRE);

  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_SUSCETIVEIS_JOVEM_FEMININO_TRAJETO,
                 SUSCETIVEL, FEMININO, JOVEM, &i, TRAJETO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_EXPOSTOS_JOVEM_FEMININO_TRAJETO, EXPOSTO,
                 FEMININO, JOVEM, &i, TRAJETO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_INFECTADOS_JOVEM_FEMININO_TRAJETO,
                 INFECTADO, FEMININO, JOVEM, &i, TRAJETO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_RECUPERADOS_JOVEM_FEMININO_TRAJETO,
                 RECUPERADO, FEMININO, JOVEM, &i, TRAJETO);

  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_SUSCETIVEIS_ADULTO_FEMININO_LOCAL,
                 SUSCETIVEL, FEMININO, ADULTO, &i, LOCAL);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_EXPOSTOS_ADULTO_FEMININO_LOCAL, EXPOSTO,
  ...
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 ...
                 RECUPERADO, FEMININO, ADULTO, &i, LOCAL);

  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_SUSCETIVEIS_ADULTO_FEMININO_ALEATORIO,
                 SUSCETIVEL, FEMININO, ADULTO, &i, ALEATORIO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_EXPOSTOS_ADULTO_FEMININO_ALEATORIO, EXPOSTO,
                 FEMININO, ADULTO, &i, ALEATORIO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_INFECTADOS_ADULTO_FEMININO_ALEATORIO,
                 INFECTADO, FEMININO, ADULTO, &i, ALEATORIO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_RECUPERADOS_ADULTO_FEMININO_ALEATORIO,
                 RECUPERADO, FEMININO, ADULTO, &i, ALEATORIO);

  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_SUSCETIVEIS_ADULTO_FEMININO_LIVRE,
                 SUSCETIVEL, FEMININO, ADULTO, &i, LIVRE);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_EXPOSTOS_ADULTO_FEMININO_LIVRE, EXPOSTO,
  ...
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 ...
                 RECUPERADO, FEMININO, ADULTO, &i, LIVRE);

  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_SUSCETIVEIS_ADULTO_FEMININO_TRAJETO,
                 SUSCETIVEL, FEMININO, ADULTO, &i, TRAJETO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_EXPOSTOS_ADULTO_FEMININO_TRAJETO, EXPOSTO,
                 FEMININO, ADULTO, &i, TRAJETO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_INFECTADOS_ADULTO_FEMININO_TRAJETO,
                 INFECTADO, FEMININO, ADULTO, &i, TRAJETO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_RECUPERADOS_ADULTO_FEMININO_TRAJETO,
                 RECUPERADO, FEMININO, ADULTO, &i, TRAJETO);

  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_SUSCETIVEIS_IDOSO_FEMININO_LOCAL,
                 SUSCETIVEL, FEMININO, IDOSO, &i, LOCAL);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_EXPOSTOS_IDOSO_FEMININO_LOCAL, EXPOSTO,
  ...
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 ...
                 RECUPERADO, FEMININO, IDOSO, &i, LOCAL);

  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_SUSCETIVEIS_IDOSO_FEMININO_ALEATORIO,
                 SUSCETIVEL, FEMININO, IDOSO, &i, ALEATORIO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_EXPOSTOS_IDOSO_FEMININO_ALEATORIO, EXPOSTO,
                 FEMININO, IDOSO, &i, ALEATORIO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_INFECTADOS_IDOSO_FEMININO_ALEATORIO,
                 INFECTADO, FEMININO, IDOSO, &i, ALEATORIO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_RECUPERADOS_IDOSO_FEMININO_ALEATORIO,
                 RECUPERADO, FEMININO, IDOSO, &i, ALEATORIO);

  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_SUSCETIVEIS_IDOSO_FEMININO_LIVRE,
                 SUSCETIVEL, FEMININO, IDOSO, &i, LIVRE);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_EXPOSTOS_IDOSO_FEMININO_LIVRE, EXPOSTO,
  ...
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 ...
                 RECUPERADO, FEMININO, IDOSO, &i, LIVRE);

  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_SUSCETIVEIS_IDOSO_FEMININO_TRAJETO,
                 SUSCETIVEL, FEMININO, IDOSO, &i, TRAJETO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_EXPOSTOS_IDOSO_FEMININO_TRAJETO, EXPOSTO,
                 FEMININO, IDOSO, &i, TRAJETO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_INFECTADOS_IDOSO_FEMININO_TRAJETO,
                 INFECTADO, FEMININO, IDOSO, &i, TRAJETO);
  inserirAgentes(quantAgentes, agentes, parametros, quantLotes, quantQuadras,
                 indexQuadras, indexPosicoes, posicoes, indexRotas, rotas,
                 indexTrajetos, trajetos, indexTrajetosFaixaEtaria,
                 QUANTIDADE_AGENTES_RECUPERADOS_IDOSO_FEMININO_TRAJETO,
                 RECUPERADO, FEMININO, IDOSO, &i, TRAJETO);
...
void salvarSaidaEspacial(std::string pastaSaida, const int *saidaEspacial,
                         const int *posicoes, int quantLinhasSaidaEspacial,
...
MARCADOR_GLOBAL void gerarSaidaQuantidadeTotal(const TIPO_AGENTE *agentes,
                                               int quantAgentes,
...
MARCADOR_GLOBAL void gerarSaidaEspacial(const TIPO_AGENTE *agentes,
...
MARCADOR_DEVICE void movimentacaoLocal(
    int id, void *seeds, TIPO_AGENTE *agentes, int quantAgentes,
...
MARCADOR_DEVICE void movimentacaoAleatoria(
    int id, void *seeds, TIPO_AGENTE *agentes, int quantAgentes,
    const int *indexQuadras, const int *indexVizinhancas,
    const int *vizinhancas, const double *parametros,
    const int *indexFronteiras, const int *fronteiras, const int *indexEsquinas,
...
MARCADOR_DEVICE void movimentacaoLivre(
    int id, void *seeds, TIPO_AGENTE *agentes, int quantAgentes,
    const int *indexQuadras, const int *indexVizinhancas,
    const int *vizinhancas, const double *parametros,
    const int *indexFronteiras, const int *fronteiras, const int *indexEsquinas,
...
MARCADOR_DEVICE void movimentacaoTrajeto(
    int id, void *seeds, TIPO_AGENTE *agentes, int quantAgentes,
    const int *indexQuadras, const int *indexVizinhancas,
    const int *vizinhancas, const double *parametros,
    const int *indexFronteiras, const int *fronteiras, const int *indexEsquinas,
          ...
          if (pontosLoteDestino > 0) {
            ...
            if (m + 1 == ((indexRotas[trajetos[indexTrajetos[t]] + r + 1] -
              ...
              if (indMenorViz != -1) {
                ...
                for (int i = indexVizinhancas[indexQuadras[2 * q] + l];
                  ...
                  if (vizinhancas[i + 0] == x && vizinhancas[i + 1] == y &&
                      ...
                      vizinhancas[i + 3] != y_min && vizinhancas[i + 4] == l &&
                      vizinhancas[i + 5] == RUA) {
                    double dist = DIST(vizinhancas[i + 2], vizinhancas[i + 3],
                                       x_destino, y_destino);
                    if (dist < distMenorViz) {
              ...
              if (distMenorViz < 2) {
                ...
                int indMenorViz = -1;
                double distMenorViz =
                    DIST(x, y, centrosEsquinas[pontoCentral + 0],
                         centrosEsquinas[pontoCentral + 1]);
                for (int i = indexVizinhancas[indexQuadras[2 * q] + l];
                     i < indexVizinhancas[indexQuadras[2 * q] + l + 1];
                     i += 6) {
                ...
                if (indMenorViz != -1) {
                  ...
                  int y_min = y;
                  // Procura na vizinhança do agente o ponto com menor distância
                  // ao ponto central da esquina que pertença a mesma rua do
                  // agente
                  int indMenorViz = 0;
                  double distMenorViz = INT_MAX;
                  for (int i = indexVizinhancas[indexQuadras[2 * q] + l];
...
MARCADOR_GLOBAL void
movimentacao(void *seeds, TIPO_AGENTE *agentes, int quantAgentes,
             const int *indexQuadras, const int *indexVizinhancas,
             const int *vizinhancas, const double *parametros,
             ...
             const int *indexPosicoes, const int *posicoes, int quantTrajetos,
             const int *indexTrajetos, const int *trajetos,
             const int *indexPeriodos, const int *periodos) {
...
MARCADOR_GLOBAL void contato(void *seeds, TIPO_AGENTE *agentes,
...
MARCADOR_GLOBAL void transicao(void *seeds, TIPO_AGENTE *agentes,
...
void iniciarSimulacao(
    ...
    std::string pastaSaida, int *saidaQuantidadeTotal, const int *quantLotes,
    int quantQuadras, const int *indexQuadras, const int *indexVizinhancas,
  ...
  int quantAgentes =
      Agentes::contarTotalAgentes(quantLotes, quantQuadras, parametros);
  TIPO_AGENTE *agentes = Agentes::criarAgentes(
      quantAgentes, parametros, quantLotes, quantQuadras, indexQuadras,
      indexPosicoes, posicoes, indexRotas, rotas, indexTrajetos, trajetos,
  ...
  for (int ciclo = 1; ciclo < ciclos; ++ciclo) {
    ...
    movimentacao<<<f1, numThreads>>>(
        seeds, agentesDev, quantAgentes, indexQuadrasDev, indexVizinhancasDev,
        vizinhancasDev, parametrosDev, indexFronteirasDev, fronteirasDev,
        indexEsquinasDev, esquinasDev, indexCentrosEsquinasDev,
        centrosEsquinasDev, quantRotas, indexRotasDev, rotasDev,
        indexPosicoesDev, posicoesDev, quantTrajetos, indexTrajetosDev,
        trajetosDev, indexPeriodosDev, periodosDev);

    contato<<<f2, numThreads>>>(seeds, agentesDev, quantAgentes, quantLotesDev,
                                quantQuadras, parametrosDev, indexQuadrasDev,
  ...
  for (int ciclo = 1; ciclo < ciclos; ++ciclo) {
    ...
    movimentacao(NULL, agentes, quantAgentes, indexQuadras, indexVizinhancas,
                 vizinhancas, parametros, indexFronteiras, fronteiras,
                 indexEsquinas, esquinas, indexCentrosEsquinas, centrosEsquinas,
                 quantRotas, indexRotas, rotas, indexPosicoes, posicoes,
                 quantTrajetos, indexTrajetos, trajetos, indexPeriodos,
                 periodos);
